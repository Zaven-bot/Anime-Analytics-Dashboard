...
import { 
  TrendingUp, Calendar, RefreshCw, AlertCircle, Activity, BarChart3,
  Users, Star, Trophy, Heart, Eye, Hash
} from 'lucide-react';
...
const TrendsChart = () => {
  ...
  const hasValidData = filteredData.some(d => d.displayValues[metric] !== null && d.displayValues[metric] !== undefined);
          ...
          <div className="space-y-6">
                ...
                <>
                  ...
                  <div className="bg-slate-800/50 rounded-lg p-3 text-center">
                    <div className="text-anime-purple font-bold text-lg">
                      {Math.max(...data.map(d => d.year)) - Math.min(...data.map(d => d.year)) + 1}
                    </div>
            ...
            {filteredData.length > 0 && (
              <div className="text-xs text-slate-500 bg-slate-800/30 rounded p-3">
                <p>
                  <strong>Note:</strong> Future seasons may show limited data (scores, rankings) as anime haven't been released yet. 
