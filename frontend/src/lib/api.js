...
export class AnimeAnalyticsAPI {
  ...
  static async getOverviewStats(snapshotType = 'top') {
    try {
      const response = await api.get(`/stats/overview?snapshot_type=${snapshotType}`);
  ...
  static async getTopRatedAnime(limit = 10, snapshotType = 'top') {
    try {
      const response = await api.get(`/anime/top-rated?limit=${limit}&snapshot_type=${snapshotType}`);
      return response.data;
    } catch (error) {
  ...
  static async getGenreDistribution(snapshotType = 'top') {
    try {
      const response = await api.get(`/anime/genre-distribution?snapshot_type=${snapshotType}`);
      return response.data;
    } catch (error) {
