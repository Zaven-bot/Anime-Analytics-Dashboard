...
from tenacity import retry, stop_after_attempt, wait_exponential, retry_if_exception_type # Retry logic
from .rate_limiter import JikanRateLimiter
from ..models.jikan import JikanSearchResponse, JikanAnime
...
class JikanExtractor:
    ...
    def __init__(self, rate_limiter: Optional[JikanRateLimiter] = None):
        ...
        self.client = httpx.AsyncClient(
            ...
            headers={
                "User-Agent": "AnimeDashboard-ETL/1.0"
            }
        )
    ...
    async def fetch_anime_search(
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    logger.error("Failed to parse Jikan response", error=str(e), page=current_page)
                    break
        ...
        logger.info("Completed anime search", total_anime=len(all_anime), pages_fetched=current_page)
        return all_anime
